(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('argon-logger'), require('silkrouter')) :
  typeof define === 'function' && define.amd ? define(['exports', 'argon-logger', 'silkrouter'], factory) :
  (global = global || self, factory(global.argon = {}, global.Logger, global.silkrouter));
}(this, (function (exports, Logger, silkrouter) { 'use strict';
  ...
  */
  ...
  var ROOT_EVENT = 'init.core';
  var INVALID_RESPONSE_OBJECT = "Promise returned an invalid response.\nFollowing are the valid signatures:\n1. { template, target, data }\n2. { template, target, error },\n3. { target, html }";
  var TARGET_MISSING = "Couldn't resolve target reference.";
  var INVALID_RENDER_OBJECT = "HTML or Promise returned by \"render\" method is invalid.\nHint: If you are using \"render.fn\" module to render components, use spread operator while combining it with regular HTML:\ne.g. return ['<div>Regular HTML</div>', ...render.fn({ ... })];";
  var TEMPLATE_MISSING = 'Target template is missing.';
  var INVALID_URL = 'Provided URL is invalid.';
  var INVALID_SCHEMA = 'Invalid schema for URL object. Allowed fields are "path" and "data".';
  ...
  function _renderHTML(response, target) {
    var $ref = $(target ? target : this.root);
  ...
  function _renderTemplate(response) {
    try {
      var target = _resolveTargetRef.apply(this, [response.target]);
  ...
  function _processResponses(response) {
    ...
    responses.forEach(function (resp) {
      if (typeof resp === 'string') {
        _renderHTML.apply(_this, [resp]);

        $body.trigger(ROOT_EVENT, [_this.root]);
      } else if (typeof resp === 'function') {
        _renderHTML.apply(_this, [resp()]);

        $body.trigger(ROOT_EVENT, [_this.root]);
      } else if (resp && _typeof(resp) === 'object') {
        if (Array.isArray(resp)) {
          throw new TypeError(INVALID_RESPONSE_OBJECT);
        }

        _renderTemplate.apply(_this, [resp]);
      }
    });
  }
  ...
  function _resolvePromise(promise) {
    ...
    promise.then(function (response) {
      _processResponses.apply(_this2, [response]);
    ...
    try {
      ...
      if (response instanceof Router) {
        ...
        response.forEach(function (resp) {
          if (typeof resp === 'string') {
            _renderHTML.apply(_this3, [resp]);
          } else if (typeof resp === 'function') {
            _renderHTML.apply(_this3, [resp()]);
          } else if (resp.then) {
            _resolvePromise.apply(_this3, [resp]);
          } else {
  ...
  function () {
    function Request(url, config) {
      ...
      this.fetch = window.fetch(url, Object.assign(config, {
        signal: signal
      }));
    }
    ...
    _createClass(Request, [{
      ...
      value: function then() {
        ...
        return (_this$fetch = this.fetch).then.apply(_this$fetch, arguments);
      }
    }, {
      key: "catch",
      value: function _catch() {
        ...
        return (_this$fetch2 = this.fetch)["catch"].apply(_this$fetch2, arguments);
      }
    }]);
  ...
  function _removeExisting(url, abort) {
    var existing = requestMap.filter(function (req) {
      var existingUrl = req.url.split('?')[0];
      var incomingUrl = url.split('?')[0];
      return existingUrl === incomingUrl;
    });
    existing.forEach(function (req) {
      if (abort) {
        req.abort();
      }
  ...
  function _request(url) {
    ...
    if (url && typeof url === 'string') {
      ...
      var currentPromise = new Request$1(url, config);
      requestMap.push(currentPromise);
      currentPromise.then(function () {
        return _removeExisting(url);
      })["catch"](function () {
        return _removeExisting(url);
      });
  ...
  function isValidObject(ob) {
    return ob && _typeof(ob) === 'object' && !Array.isArray(ob);
  }
  /**
    ...
    if (typeof data !== 'string') {
      if (data && _typeof(data) === 'object') {
        if (isContentTypeJson(config)) {
          ...
          url.data = silkrouter.param(data);
        }
      }
    }
  }
  ...
  function resolveURL(path) {
    ...
    } else if (path && _typeof(path) === 'object') {
      if (Array.isArray(path)) {
        return path.map(function (url) {
          if (typeof url === 'string') {
            ...
            return validateSchema.apply(_this, [url]);
          }
        });
  ...
  function renderData() {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (template) {
        if (_this2.templateMap[template]) {
          resolve(Object.assign(responseObj, {
            template: _this2.templateMap[template],
            data: config.data
          }));
        } else {
  ...
  function resolveToArray(inputStr, length, type) {
    var error = type === 'template' ? INVALID_TEMPLATE_NAME : INVALID_SELECTOR;

    if (inputStr && typeof inputStr === 'string' || type === 'target' && isValidSelector(inputStr)) {
      var list = [];
      ...
      return inputStr.map(function (str) {
        if (str && typeof str === 'string' || type === 'target' && isValidSelector(str)) {
          return str;
        }
  ...
  function doRequest(URL, method, fetchOptions) {
    if ([methods$1.GET, methods$1.DELETE].includes(method)) {
      var requestPath = URL.data ? "".concat(URL.path, "?").concat(URL.data) : URL.path;
      return request(requestPath, Object.assign({}, fetchOptions));
    } else {
      return request(URL.path, Object.assign({}, fetchOptions, {
        body: URL.data
      }));
    }
  }
  ...
  function sequence(returnPromises, inputTemplates, targets, upResolve, upReject) {
    ...
    if (URLs.length >= 1) {
      var method = fetchOptions.method || methods$1.GET;
      returnPromises.push(new Promise(function (resolve, reject) {
        var current = URLs.splice(0, 1)[0];
        var template = inputTemplates.splice(0, 1)[0];
        var target = targets.splice(0, 1)[0];
        doRequest(current, method, fetchOptions).then(function (res) {
          return res.json();
        }).then(function (data) {
          renderAjaxData.apply(_this4, [template, target, data, resolve, reject]);
          sequence.apply(_this4, [returnPromises, inputTemplates, targets, upResolve, upReject]);
        })["catch"](function (error) {
          renderError.apply(_this4, [_this4.templateMap[template], target, error, reject]);
        });
      }));
    } else {
      Promise.all(returnPromises).then(upResolve)["catch"](upReject);
    }
  }
  ...
  function renderAjax() {
    ...
    try {
      ...
      if (this.URLs.length) {
        ...
        return [new Promise(function (resolve, reject) {
          sequence.apply(_this5, [[], template, target, resolve, reject]);
        })];
      }
  ...
  function diff(newImports, currentRoot) {
    var existingImports = componentImports.filter(function (imp) {
      return imp.parent === currentRoot;
    });
    existingImports.forEach(function (imp) {
    ...
    var _loop = function _loop() {
      ...
      var curr = newImports.splice(0, 1);
      var prev = existingImports.filter(function (imp) {
        return imp.root === curr[0].root && imp.name === curr[0].name;
      });

      if (prev.length) {
        prev.forEach(function (imp) {
          componentImports.push.apply(componentImports, _toConsumableArray(existingImports.splice(existingImports.indexOf(imp), 1)));
        });
      } else {
        componentImports.push.apply(componentImports, _toConsumableArray(curr));
      }
    };
    ...
    if (existingImports.length) {
      ...
      while (true) {
        ...
        existingImports.splice(0, 1).forEach(function (imp) {
          if (imp.ref && typeof imp.ref.doDestroy === 'function') {
            imp.ref.doDestroy();
            logger.log("[Webpack]: component \"".concat(imp.name, "\" has been destroyed."));
          }
        });
      }
    }

    return componentImports.filter(function (imp) {
      return !imp.imported;
    });
  }
  ...
  function initializeModule() {
    ...
    currentRoot.find('[data-module]').each(function (el) {
      ...
      if (typeof dataModule === 'string') {
        dataModule.split(',').forEach(function (mod) {
          mod = mod.trim();

          if (!newImports.filter(function (imp) {
            return imp.root === el && imp.name === mod;
          }).length) {
            newImports.push({
              root: el,
              parent: root,
